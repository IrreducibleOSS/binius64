...
fn process_message(
	...
	for i in 0..MAX_BLOCKS {
		...
		let block_counter = builder.select(block_counter_normal, message_length, is_last);
		...
		for j in 0..8 {
			h[j] = builder.select(h_before[j], h[j], is_active);
		}
	}
}
...
pub fn compress(
	...
	v[14] = builder.select(v[14], inverted, last_block_flag);
