...
impl IdentityCommitmentECDSA {
    fn new(builder: &CircuitBuilder, curve: &Secp256k1) -> Self {
        ...
        let commitment: [Wire; KECCAK_DIGEST_LIMBS] = std::array::from_fn(|_| builder.add_witness());
        let message_len = builder.add_constant_64(PUBKEY_MESSAGE_BYTES as u64);
        
        let hasher = Keccak::new(builder, message_len, commitment, commitment_message_wires.clone());
    ...
    fn populate_witness(
        ...
        pack_bytes_into_wires_le(witness, &self.commitment_message_wires, &commitment_message_bytes);
        
        let commitment_digest = Keccak256::digest(&commitment_message_bytes);
        self.hasher.populate_len_bytes(witness, commitment_message_bytes.len());
        self.hasher.populate_message(witness, &commitment_message_bytes);
        self.hasher.populate_digest(witness, commitment_digest.into());
    }
}
...
impl MerkleProofKeccak {
    fn new(
        ...
        for sibling in &siblings {
            ...
            let left: [Wire; KECCAK_DIGEST_LIMBS] = multi_wire_multiplex(
                builder,
                &[sibling, &current],
                is_even,
            ).try_into().unwrap();
            
            let right: [Wire; KECCAK_DIGEST_LIMBS] = multi_wire_multiplex(
                builder,
                &[&current, sibling],
                is_even,
            ).try_into().unwrap();
            ...
            let parent: [Wire; KECCAK_DIGEST_LIMBS] = std::array::from_fn(|_| builder.add_witness());
...
impl NullifierGeneratorECDSA {
    fn new(
        ...
        let secret_scalar_wires: [Wire; SCALAR_LIMBS] = std::array::from_fn(|_| builder.add_witness());
    ...
    fn populate_witness(
        ...
        self.hasher.populate_len_bytes(witness, scope.len() + PUBKEY_COORD_BYTES);
...
mod tests {
    ...
    use crate::circuits::semaphore_ecdsa::reference::{MerkleTree, SemaphoreProofECDSA as RefProof};
    ...
    fn test_circuit_matches_reference() {
        ...
        let reference_proof = RefProof::generate(
            &identity, &tree, 0, message, scope
        );
        ...
        let circuit_nullifier: [u8; 32] = circuit.nullifier.iter()
            .flat_map(|&w| witness[w].0.to_le_bytes())
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();
        
        assert_eq!(circuit_nullifier, reference_proof.nullifier,
                   "Circuit nullifier should match reference");
        
        // Extract and verify Merkle root matches
        let circuit_root: [u8; 32] = circuit.merkle_root.iter()
            .flat_map(|&w| witness[w].0.to_le_bytes())
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();
        
        assert_eq!(circuit_root, tree.root(),
                   "Circuit root should match reference");
    }
    ...
    fn test_identity_commitment_matches_reference() {
        ...
        assert_eq!(proof.leaf, reference_commitment,
                   "Identity commitment should match reference");
    }
    ...
    fn test_nullifier_consistency() {
        ...
        assert_eq!(circuit_nullifier, reference_nullifier,
