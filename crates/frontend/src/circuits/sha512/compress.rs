...
impl Compress {
	...
	pub fn populate_m(&self, w: &mut crate::compiler::circuit::WitnessFiller, m: [u8; 128]) {
...
fn small_sigma_0(b: &CircuitBuilder, x: Wire) -> Wire {
	let r1 = b.rotr_64(x, 1);
	let r2 = b.rotr_64(x, 8);
	let s1 = b.shr(x, 7);
	let x1 = b.bxor(r1, r2);
	b.bxor(x1, s1)
}
...
fn small_sigma_1(b: &CircuitBuilder, x: Wire) -> Wire {
	let r1 = b.rotr_64(x, 19);
	let r2 = b.rotr_64(x, 61);
	let s1 = b.shr(x, 6);
	let x1 = b.bxor(r1, r2);
	b.bxor(x1, s1)
}
...
mod tests {
	...
	fn sha512_chain() {
		...
		println!("{N} sha512 compress512 invocations");
	...
	fn sha512_parallel() {
		...
		println!("{N} sha512 compress512 invocations in parallel");
