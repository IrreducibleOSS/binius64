...
impl Sha512 {
	...
	pub fn new(
		...
		let (len_plus_16, _carry) = builder.iadd_cin_cout(len, builder.add_constant_64(16), zero);
		let end_block_index = builder.shr(len_plus_16, 7);
		...
		for word_index in 0..n_words {
			...
			// for both me and pep, `n_words` represents the number of "dense" 64-bit words' worth of info.
			// for pep, `padded_message` has a different semantics: there are 16 64-bit words per element,
			// but each is only filled halfway, it seems. so he wants to read two words at a time.
			// so the total content of `padded_message` is twice as much as `n_words`.
			// for us, `padded_message` is a 2d array; each element is a 16-element array of 64-bit words.
