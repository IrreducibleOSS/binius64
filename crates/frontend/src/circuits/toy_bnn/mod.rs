...
impl ToyBNN {
	...
	pub fn populate_i(&self, filler: &mut WitnessFiller<'_>, i_values: Vec<u64>) {
		assert_eq!(i_values.len(), self.i_values.len(), "Input values count mismatch");
...
pub fn parse_binary_file<P: AsRef<Path>>(
	path: P,
...
mod tests {
	...
	fn test_with_actual_files() {
		...
		let module_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/circuits/toy_bnn");
		...
		for (i, &value) in output_values.iter().enumerate() {
			assert!(value >= 55, "Output {} should be >= 55, got {}", i, value);
		}
		...
		let avg_popcount: f64 =
			output_values.iter().sum::<u64>() as f64 / output_values.len() as f64;
		...
		for i in 0..5.min(output_values.len()) {
			println!(
				"  I[{}]={:09b} XOR W={:09b} -> XNOR has {} bits set",
	...
	fn test_xnor_popcount_properties() {
		...
		for (w_val, i_val) in test_cases {
			...
			circuit
				.populate_wire_witness(&mut filler)
				.unwrap_or_else(|_| {
					panic!("Circuit should work for W={:09b}, I={:09b}", w_val, i_val)
				});
