...
fn test_depth_limit_deep_chain() {
	...
	// Build a chain deeper than MAX_DEPTH (which is 5 ATOW)
	// Each level adds more complexity through XOR operations
	// The optimizer should commit intermediate values when depth > 5
	...
	// Level 6: XOR with v5 - this should exceed MAX_DEPTH
	let w6 = b.add_witness();
	let v6 = b.bxor(v5, w6);

	// Level 7: XOR with v6 - definitely beyond MAX_DEPTH
	let w7 = b.add_witness();
	let v7 = b.bxor(v6, w7);

	// Use v7 in an AND constraint to trigger the fusion optimization
	let w8 = b.add_witness();
	let _result = b.band(v7, w8);
	...
	insta::assert_snapshot!(stringify_constraint_system(&cs), @r"
	AND[0]: (v[11] ⊕ v[4] ⊕ v[5] ⊕ v[6] ⊕ v[7] ⊕ v[8] ⊕ v[9]) ∧ (v[10]) = (v[12])
	AND[1]: (v[2] ⊕ v[3]) ∧ (all-1) = (v[11])
	");
}
...
fn test_depth_limit_with_shifts() {
	...
	insta::assert_snapshot!(stringify_constraint_system(&cs), @r"
	AND[0]: (v[11]≪7 ⊕ v[5]≪7 ⊕ v[6] ⊕ v[7] ⊕ v[8] ⊕ v[9]) ∧ (v[10]) = (v[12])
	AND[1]: (v[2]≫3 ⊕ v[3]≫3 ⊕ v[4]) ∧ (all-1) = (v[11])
	");
}
