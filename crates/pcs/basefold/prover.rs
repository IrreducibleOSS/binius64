	...
	<F as WithUnderlier>::Underlier: PackScalar<FA>,
{
	...
	pub fn new(
		...
		let fri_folder =
			FRIFolder::new(fri_params, ntt, merkle_prover, committed_codeword, committed).unwrap();
	...
	pub fn fold(
		&mut self,
		challenge: F,
	) -> Result<FoldRoundOutput<<VCS as MerkleTreeScheme<F>>::Digest>, binius_prover::fri::Error> {
