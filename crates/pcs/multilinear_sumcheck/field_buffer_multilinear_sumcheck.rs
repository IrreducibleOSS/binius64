...
impl<BF: Field> NikitaSumcheckProver<BF> for MultilinearSumcheckProver<BF> {

    fn n_vars(&self) -> usize {
        self.log_n
    }

    fn execute(&mut self) -> Result<Vec<RoundCoeffs<BF>>, Error> {
    ...
    fn fold(&mut self, challenge: BF) -> Result<(), Error> {
        match self.fold_direction {
            FoldDirection::HighToLow => {
    ...
    fn finish(self) -> Result<Vec<BF>, Error> {
        Ok(vec![    
...
pub mod test {
    ...
    fn two_random_field_buffers(log_n: usize) -> [FieldBuffer<BF>; 2] {
        let mut rng = StdRng::from_seed([0; 32]);
        let n = 1 << log_n;
        array::from_fn(|_| FieldBuffer::from_values(   
    ...
    pub fn sumcheck_interactive_protocol(prover: &mut MultilinearSumcheckProver<BF>) -> Result<(BF, Vec<BF>), Error> {
    ...
    fn test_sumcheck_interactive_protocol(
        ...
        let _final_eval_claims = prover.finish()?;
    ...
    fn test_sumcheck_low_to_high() {
        ...
        test_sumcheck_interactive_protocol(
            multilinears, 
            FoldDirection::LowToHigh
        ).unwrap();
    }   
