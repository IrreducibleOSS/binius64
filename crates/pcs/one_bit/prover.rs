...
use crate::utils::utils::{compute_expected_sumcheck_claim, construct_s_hat_u};
use crate::{
...
use binius_field::{
    as_packed_field::PackScalar, underlier::WithUnderlier, BinaryField, ExtensionField, Field,
...
use binius_transcript::fiat_shamir::{Challenger, CanSample};
    ...
    BigField: TowerField + From<u128> + PackedExtension<SmallField>,
{
        ...
        <BigField as WithUnderlier>::Underlier: PackScalar<FA>,
    {
        ...
        transcript
            .message()
            .write_scalar_slice(&prover_s_hat_v);
    ...
    fn initialize_proof(
        ...
        for (packed_elem, eq_at_high_value) in small_field_mle.iter().zip(eq_at_high.as_ref()){
            packed_elem.iter().enumerate().for_each(|(low_vars_subcube_idx,bit_in_packed_field)|{
