...
use binius_verifier::and_reduction::{utils::{subfield_isomorphism::SubfieldIsomorphismLookup, constants::{ROWS_PER_HYPERCUBE_VERTEX, SKIPPED_VARS}}, univariate::univariate_lagrange::{lexicographic_lagrange_numerators_polyval, lexicographic_lagrange_denominator}};
...
where F: Field{
    ...
    let mut lookup_table =
        vec![vec![F::ZERO; 1 << 8]; ROWS_PER_HYPERCUBE_VERTEX / 8];
