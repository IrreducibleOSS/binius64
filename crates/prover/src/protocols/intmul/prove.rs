...
use super::{error::Error, execute::LayersIterator};
use crate::protocols::sumcheck::{
	batch::batch_prove,
	bivariate_mle::BivariateMlecheckProver,
...
fn prove_phase_3<F: Field, P: PackedField<Scalar = F>, C: Challenger>(
	...
	let claims = izip!(twisted_eval_points, twisted_claims)
		.map(|(point, value)| Claim { point, value })
		.collect_vec();

	let v_prover = SelectorMlecheckProver::new(v_buffer.clone(), claims, b_exponents, 0)
		.map_err(Error::from_sumcheck_new)?;
	...
	Ok(make_phase_3_output(challenges.clone(), v_prover_claims, c_prover_claims))
}
