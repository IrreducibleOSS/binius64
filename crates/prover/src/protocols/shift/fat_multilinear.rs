...
/// Constructs the "fat multilinear" that combines all shift operations into a single
/// multilinear.
///
...
#[instrument(skip_all, name = "build_fat_multilinear")]
pub fn build_fat_multilinear<F: BinaryField, P: PackedField<Scalar = F>>(
	...
	let fat_multilinear = key_collection
		.key_ranges
		.par_iter()
		.map(|Range { start, end }| {
	...
	let log_len = strict_log_2(fat_multilinear.len())
		.expect("same length as constraint system's `key_ranges`");
	Ok(FieldBuffer::new(log_len, fat_multilinear).unwrap())
}
