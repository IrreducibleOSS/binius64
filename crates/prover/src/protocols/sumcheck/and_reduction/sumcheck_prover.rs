...
pub struct AndReductionMultilinearSumcheckProver<F: Field> {
...
impl<F: Field> AndReductionMultilinearSumcheckProver<F> {
    pub fn new(
        multilinears: Vec<FieldBuffer<F>>,
        zerocheck_challenges: Vec<F>,
        overall_claim: F,
...
impl<F: Field> SumcheckProver<F> for AndReductionMultilinearSumcheckProver<F> {
    ...
    fn fold(&mut self, sumcheck_challenge: F) -> Result<(), Error> {
        ...
        println!("\n set current round claim:: sumcheck_challenge: {:?}", sumcheck_challenge);
        println!("\n set current round claim:: zerocheck_challenge: {:?}", zerocheck_challenge);
        println!("\n set current round claim:: round_message: {:?}", round_message);
        ...
        println!("\n set current round claim:: current round claim: {:?}", self.current_round_claim);
    ...
    fn execute(&mut self) -> Result<Vec<RoundCoeffs<F>>, Error> {
        ...
        println!("\nexecute:: eq factor: {:?}", self.eq_factor);
        println!("\nexecute:: current round claim: {:?}", self.current_round_claim);

        println!("\n g(0) + g(1): {:?}", g_of_zero + g_of_one);
...
pub fn multilinear_sumcheck<F: Field>(
    mut prover: AndReductionMultilinearSumcheckProver<F>,
) -> (F, Vec<F>) {
...
pub mod test {
    ...
    fn sumcheck_four_column(fold_direction: FoldDirection) {
        ...
        let zerocheck_challenges = (0..log_n)
            ...
            let overall_claim = AndReductionMultilinearSumcheckProver::<BF>::sum_composition(
                &multilinears[0],
                &multilinears[1],
                &multilinears[2],
                &eq_r,
    ...
    fn test_composition_even_odd_sum() {
        ...
        let overall_sum = AndReductionMultilinearSumcheckProver::sum_composition(
            &multilinears[0],
            &multilinears[1],
            &multilinears[2],
            &multilinears[3],
