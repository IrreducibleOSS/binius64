use crate::{
    ...
    protocols::sumcheck::and_reduction::univariate::ntt_lookup::NTTLookup,
    protocols::sumcheck::and_reduction::univariate::delta::delta_poly,
    protocols::sumcheck::and_reduction::univariate::univariate_poly::{
        GenericPo2UnivariatePoly, UnivariatePoly,
    },
        ...
        .map(|subcube_idx| {
            ...
            for point_idx_within_subcube in 0..1 << 3 {
                ...
                for (i, this_byte_lookup) in ntt_lookup.iter().enumerate() {
                    let column_ntted =
                        ...
                        &this_byte_lookup[
                            ...
                            Into::<u8>::into(get_packed_slice(first_col_bytes, byte_offset + i)) as usize];
                ...
                for (i, this_byte_lookup) in ntt_lookup.iter().enumerate() {
                    let column_ntted =
                        &this_byte_lookup[
                            Into::<u8>::into(get_packed_slice(second_col_bytes, byte_offset + i)) as usize];
                ...
                for (i, this_byte_lookup) in ntt_lookup.iter().enumerate() {
                    let column_ntted =
                        &this_byte_lookup[
                            Into::<u8>::into(get_packed_slice(third_col_bytes, byte_offset + i)) as usize];
...
mod test {
    ...
    use crate::{
        ...
        protocols::sumcheck::and_reduction::one_bit_multivariate::OneBitMultivariate,
        protocols::sumcheck::and_reduction::univariate::subfield_isomorphism::SubfieldIsomorphismLookup,
        protocols::sumcheck::and_reduction::sumcheck_round_message::sum_claim, 
