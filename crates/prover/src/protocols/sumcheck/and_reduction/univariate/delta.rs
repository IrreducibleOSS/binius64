use binius_field::{arithmetic_traits::InvertOrZero, Field};
...
// #[cfg(test)]
mod tests {
    use binius_field::{AESTowerField8b, BinaryField128bPolyval, Field, AESTowerField128b};
    use crate::{
        protocols::sumcheck::and_reduction::univariate::{
    ...
    fn delta_satisfies_definition() {
        let iso_lookup = SubfieldIsomorphismLookup::<BinaryField128bPolyval>::new::<AESTowerField128b>();
        for i in 0..64 {
            let poly = delta_poly(
                iso_lookup.lookup_8b_value(AESTowerField8b::new(i)),
                6,
                &iso_lookup,
            );
            for j in 0..64 {
