...
/// An [`MleCheckProver`] implementation for the composition $(A \cdot B - 1) \cdot A$.
///
/// This composite arises in the reduction of zero-check claims that $A$'s evaluations are equal to
/// the multiplicative inverse of $B$'s evaluations, but additionally mapping 0 to 0.
/// zero-check claims that  to evaluation claims on the constituent multilinears.
///
...
#[derive(Debug, Clone)]
pub struct InverseOrZeroMleCheckProver<P: PackedField> {
	/// Invariant: the length of both multilinears is always equal.
	multilinears: [FieldBuffer<P>; 2],
	last_coeffs_or_eval: RoundCoeffsOrEval<P::Scalar>,
	gruen34: Gruen34<P>,
}

impl<F: Field, P: PackedField<Scalar = F>> InverseOrZeroMleCheckProver<P> {
...
impl<F, P> SumcheckProver<F> for InverseOrZeroMleCheckProver<P>
where
	F: Field,
...
impl<F, P> MleCheckProver<F> for InverseOrZeroMleCheckProver<P>
where
	F: Field,
	P: PackedField<Scalar = F>,
{
...
mod tests {
	...
	#[test]
	fn test_inverse_or_zero_mlecheck() {
		type F = OptimalB128;
		...
		// Create the prover
		let mlecheck_prover = InverseOrZeroMleCheckProver::new(
