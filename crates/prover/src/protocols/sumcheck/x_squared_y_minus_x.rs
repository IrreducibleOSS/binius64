...
#[derive(Debug)]
pub struct XSquaredYMinusXSumcheckProver<P: PackedField> {
	multilinears: [FieldBuffer<P>; 2],
	last_coeffs_or_sum: RoundCoeffsOrSum<P::Scalar>,
}

impl<F: Field, P: PackedField<Scalar = F>> XSquaredYMinusXSumcheckProver<P> {
...
impl<F: Field, P: PackedField<Scalar = F>> SumcheckProver<F> for XSquaredYMinusXSumcheckProver<P> {
	...
	fn execute(&mut self) -> Result<Vec<RoundCoeffs<F>>, Error> {
		...
		// We compute the monomial basis coefficients of the round polynomial directly
		let round_evals =
			(evals_a_0.as_ref(), evals_a_1.as_ref(), evals_b_0.as_ref(), evals_b_1.as_ref())
				...
				.map(|(&evals_a_0_i, &evals_a_1_i, &evals_b_0_i, &evals_b_1_i)| {
					...
					[
						a_deg0_sq * b_deg0 + a_deg0,
						a_deg0_sq * b_deg1 + a_deg1,
						a_deg1_sq * b_deg0,
					]
				})
				.reduce(|| [P::zero(); 3], |lhs, rhs| array::from_fn(|i| lhs[i] + rhs[i]));
		let mut all_coeffs_but_leading = round_evals
			.iter()
			.map(|packed_coeffs| packed_coeffs.iter().sum())
			.collect_vec();

		let leading_coeff = *last_sum + all_coeffs_but_leading[1] + all_coeffs_but_leading[2];
		all_coeffs_but_leading.push(leading_coeff);

		let round_coeffs = RoundCoeffs(all_coeffs_but_leading);
...
mod tests {
	...
	fn test_x2y_minus_x_sumcheck() {
		...
		let expected_sum = iter_packed_slice_with_offset(multilinear_a.as_ref(), 0)
			.zip(iter_packed_slice_with_offset(multilinear_b.as_ref(), 0))
			.map(|(a, b)| a.square() * b - a)
			.sum();

		// Create the prover
		let prover = XSquaredYMinusXSumcheckProver::new(
		...
		let sumcheck_output =
			verify::<F, _>(n_vars, 3, expected_sum, &mut verifier_transcript).unwrap();
