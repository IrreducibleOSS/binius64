//! Demonstration of precise type semantics in Spark
//!
...
//!
//! Run with: cargo run --example type_demo --release
...
fn main() {
    println!("=== Spark Type Semantics Demonstration ===\n");
...
fn demonstrate_field_semantics() {
    ...
    let c = ctx.witness_field(Word(0x9));
    let field_total = ctx.add(field_sum, c);
...
fn demonstrate_integer_semantics() {
    ...
    let a = ctx.witness_uint(Word(0xFFFFFFFFFFFFFFFF)); // MAX_U64
    let b = ctx.witness_uint(Word(1));
    ...
    let a_low = ctx.witness_uint(Word(0xFFFFFFFFFFFFFFFF));
    let a_high = ctx.witness_uint(Word(0));
    let b_low = ctx.witness_uint(Word(1));
    let b_high = ctx.witness_uint(Word(0));
...
fn demonstrate_bit_pattern_semantics() {
    ...
    let a = ctx.witness_bits(Word(0xFF00FF00FF00FF00));
    let b = ctx.witness_bits(Word(0x00FF00FF00FF00FF));
    let and_result = ctx.and(a, b);
    ...
    let signed_val = ctx.witness_bits(Word(0x8000000000000000)); // Negative in signed
    ...
    let bool_true = ctx.witness_bits(Word::ALL_ONE); // -1 as signed
...
fn demonstrate_type_transitions() {
    ...
    let values = [
        ctx.witness_uint(Word(10)),
        ctx.witness_uint(Word(20)),
        ctx.witness_uint(Word(30)),
    ];
    
    let selections = [
        ctx.witness_bits(Word::ALL_ONE),  // true
        ctx.witness_bits(Word::ZERO),      // false
        ctx.witness_bits(Word::ALL_ONE),  // true
    ];
    ...
    for i in 0..3 {
        ...
        let values_as_bits = ctx.as_bits_from_uint(values[i]);
        let masked = ctx.and(values_as_bits, mask);
        println!("  values[{}] & mask = {} (bit pattern)", i, masked.value.0);
        
        // Type: bit pattern -> unsigned int (context change!)
        let masked_as_uint = ctx.as_uint(masked);
        let (new_sum, _) = ctx.add_with_carry(sum, masked_as_uint, zero);
