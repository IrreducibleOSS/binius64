...
/// Evaluates the term of the fat multilinear corresponding to an operand of an operation.
///
...
/// up the term of the fat multilinear corresponding to this operand as follows:
/// $$
...
fn evaluate_fat_multilinear_term_for_operand<F: Field>(
...
/// The fat multilinear for an operation is written
/// $$
...
/// as $r_j$, $r_s$, and $r_y$.
pub fn evaluate_fat_multilinear_for_operation<F: BinaryField, const ARITY: usize>(
	...
	let eval = operand_vecs
		...
		.map(|(i, operand_vec)| {
			operator_data.lambda.pow([i as u64 + 1])
				* evaluate_fat_multilinear_term_for_operand(
					operand_vec,
					h_op_evals,
