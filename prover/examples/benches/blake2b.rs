...
fn bench_blake2b_hash(c: &mut Criterion) {
	...
	let params_list = vec![
		...
		("Note".to_string(), "Circuit size grows with max_bytes parameter".to_string()),
	];
	bench_utils::print_benchmark_header("Blake2b", &params_list);

	let params = Params {
		max_msg_len_bytes: config.max_bytes,
	};
