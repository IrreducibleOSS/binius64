...
/// Benchmark SHA-256 hash circuit
fn bench_sha256_hash(c: &mut Criterion) {
	...
	bench_utils::print_benchmark_header("SHA-256", &params_list);
	...
	let example = Sha256Example::build(params.clone(), &mut builder).unwrap();
	...
	{
		let mut group = c.benchmark_group("sha256_witness_generation");
