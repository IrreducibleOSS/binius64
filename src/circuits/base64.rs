...
fn ascii_to_val(b: &CircuitBuilder, ch: Wire) -> Wire {
    ...
    let mut out = select(
        b,
        add_const(b, ch, (!('A' as u32)).wrapping_add(1)),
        zero,
        {
    ...
    out = select(
        b,
        add_const(b, ch, (!('a' as u32)).wrapping_add(1 + 26)),
        out,
        {
            let ge = gt_const(b, ch, ('a' as u32) - 1);
            let le = bool_not(b, gt_const(b, ch, 'z' as u32));
    ...
    out = select(
        b,
        add_const(b, ch, (!('0' as u32)).wrapping_add(1 + 52)),
        out,
        {
    ...
    out = select(
        b,
        b.add_constant(Word(62)),
        out,
        eq_const(b, ch, '+' as u32),
    );

    // '/' -> 63
    select(
        b,
        b.add_constant(Word(63)),
        out,
        eq_const(b, ch, '/' as u32),
    )
}
