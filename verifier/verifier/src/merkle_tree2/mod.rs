...
/// - The outermost layer consists of hashes of **batches** of leaves, the batch size is determined by `log_batch_size`.
/// - The innermost layer is _not_ the root; instead the tree is cut before it reaches the root, and the innermost layer is `commit_layer`.
///   This serves as the commitment.
///
/// Generics:
/// - `T`: The leaf type. (_Not_ the type of a leaf batch. A leaf batch is a slice of elements of type `T`.)
...
pub struct MerkleTreeVerifier<T, H: OutputSizeUser, C> {
	...
	compression: C,
	/// The leaves are only stored in the prover. But we want to have the generic `T` on the verifier, which is why we need to add `PhantomData`.
...
{
	/// Reads a merkle tree commitment. Returns a verifier instance which allows to verify openings using [`Self::verify_opening`].
	///
	/// Arguments:
	/// - `compression`: The compression function used to construct inner nodes.
	/// - `log_leaves`: Base-2 logarithm of the number of raw leaves. Note that a leaf batch (which is an "actual leaf of the tree") consists of a slice of leaves.
