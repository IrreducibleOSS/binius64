	...
	F: BinaryField + From<AESTowerField8b>,
{
	...
	let eval = iter::zip(evals, h_op_evals)
		.map(|(mut evals_op, h_op)| {
			let evals_op = FieldBuffer::new(LOG_WORD_SIZE_BITS, &mut evals_op[..])
				.expect("evals_op is an array with length 2^LOG_WORD_SIZE_BITS");
			let mat_eval_op = evaluate_inplace(evals_op, r_s)
				.expect("precondition: r_s length is LOG_WORD_SIZE_BITS");

			// TODO: This is insecure!! There's no random batching for the operation.
			mat_eval_op * h_op
		})
		.sum();
